# C语言教学演示

一个面向 C 语言初学者的**交互式教学网站**：每个知识点都配有可逐步执行的可视化 Demo，直观展示代码运行时的变量状态、内存变化、调用栈和执行流程；每章末尾附有自动评测的练习题。

**在线浏览（无需下载）**：https://fanzipei.github.io/c-teaching/

纯静态站点：无框架、无构建步骤，下载后双击 `index.html` 也可离线使用。

## 章节内容

| 章节 | 内容 |
| --- | --- |
| 认识C程序 | 从 Hello World 开始：输出、变量、计算、循环与判断的初体验 |
| 数据类型与赋值语句 | 基本类型、printf/scanf、类型转换、复合赋值、自增自减、常量与宏 |
| 条件语句 | if-else、switch、三目运算符、短路求值、嵌套判断 |
| 循环语句 | while、for、do-while、break、continue、嵌套循环（含动态程序流程图） |
| 数组1 | 一维数组定义、遍历、排序、逆序；字符串连接/比较/查找、数字统计（10 个演示、4 道练习） |
| 数组2 | 多维数组、矩阵转置、行列求和、邻接矩阵；数组实现栈与顺序队列（7 个演示、4 道练习） |
| 函数 | 函数定义、返回值、作用域、递归、一维/二维数组参数、栈与队列综合应用 |
| 指针1 | 取地址与解引用、指针运算、数组访问、地址共享、查找元素与参数传递（7 个演示、6 道练习） |
| 指针2 | 二维数组与指针、多级指针、字符数组与字符串常量、动态分配、堆栈生命周期、形参与实参（9 个演示、6 道练习） |
| 结构体 | 结构体定义、结构体数组、结构体指针、嵌套、typedef、链表、malloc 创建 |
| 文件操作 | fopen/fclose、文本写入、逐字符读取、追加、fprintf/fscanf（4 个演示、4 道练习） |

## 功能特性

- **逐步执行的代码可视化**：单步/自动播放/回退，高亮当前执行行，同步展示变量、内存、数组、矩阵、调用栈、链表、图等状态
- **动态程序流程图**（条件/循环章节）：实时高亮当前节点与执行路径，箭头按节点形状贴合边缘，复杂图在小屏幕上可横向查看
- **scanf 交互模拟**：像真实终端一样输入一整行数据，观察格式串逐项解析与经典陷阱
- **章节练习**：预测输出题与改错题；数组1、数组2、文件操作各 4 道，其余非入门章节各 6 道，代码语法高亮、自动评测，答错后分层提示并可主动查看参考答案
- **学习进度**：demo 与练习的完成情况记录于浏览器 localStorage，首页可继续未完成项，章内导航同步完成状态
- **学生反馈入口**：每页导航栏均可一键打开预填页面信息的 GitHub Issue，便于报告内容错误、演示问题或提出建议
- 支持键盘快捷键（←/→ 单步、空格播放暂停、Home 重置）、屏幕阅读器与减少动画偏好，适配移动端

## 本地运行

直接用浏览器打开 `index.html` 即可，无需任何依赖。

如需运行自动化校验（见下文），则需要 Node.js 并执行：

```bash
npm install
```

## 自动化校验工具（tools/）

```bash
node tools/regression.js    # 全站回归：跑完所有 demo、练习判题、进度持久化、首页徽章
node tools/verify_quiz.js   # 用 gcc 实际编译运行，校验全部练习题的参考答案（需 PATH 中有 gcc）
node tools/check_mobile.js  # 移动端（390px 视口）横向溢出体检
node tools/accessibility.js # 地标、控件名称、动态播报与章节导航无障碍检查
node tools/check_flowcharts.js # 流程图箭头、标签边界和连线穿越检查
node tools/_check_if.js       # 检查所有 demo 中的 if 条件行是否都被步骤覆盖
node tools/verify_steps.js    # 全站逐步状态、回退、输入分支与实际 C 输出校验（需 gcc）
node tools/bump_version.js  # 修改 style.css / demo-engine.js 后统一升级 ?v= 缓存版本号

# 或一次运行全部浏览器回归
npm test
```

## 技术说明

- 纯 HTML/CSS/JavaScript，无任何前端框架
- 全站章节配置 `site-config.js`：导航、首页进度与自动化测试共用同一份章节元数据
- 演示引擎 `demo-engine.js`：注入公共导航栏，提供 `createDemo()`（步骤式演示）与 `createQuiz()`（练习评测）两个接口
- 测试基于 [Playwright](https://playwright.dev/)，练习题答案用 gcc 实机验证

## 分步演示的维护

每一步高亮正在执行的语句，右侧显示该语句执行后的状态。声明、循环初始化、条件判断、循环更新、函数调用和返回分别成步；未执行的分支不会出现。预处理指令和独立花括号不作为执行步骤。

88 个固定输入样例的步骤保存在 `demo-traces.js`，由页面中的 C 源码经 GCC 实际执行生成；4 个交互输入样例在页面中根据输入构造步骤。修改固定样例的 C 代码后，运行：

```bash
python -m pip install pycparser
npm run build:traces
npm run test:steps
node tools/bump_version.js
```

生成器仅在维护时使用 Python、pycparser 和 GCC，网站浏览仍为纯静态且支持离线。编译和文件读写都在临时目录中执行。`test:steps` 会检查源代码指纹，避免修改代码后遗漏重建步骤；同时逐步核对可视化状态和回退，并将终端输出与独立编译的原程序比较。

数组章节入口为 `array1.html` 和 `array2.html`；旧 `array.html` 链接（含演示、练习锚点）自动跳转，旧章节的已完成记录会迁移到对应的新位置。

## 许可证

本项目采用 [Apache License 2.0](LICENSE) 许可证。
